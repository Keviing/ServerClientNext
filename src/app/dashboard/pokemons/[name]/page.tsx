...
interface PokemonsNameProps {
    params: Promise<{ name: string }>
}
export async function generateStaticParams(){

  const date:PokemonsResponse = await fetch( `https://pokeapi.co/api/v2/pokemon?limit=151`).then(res => res.json());

  const static151Pokemons = date.results.map((pokemon) => ({name: pokemon.name}))

  return static151Pokemons.map(({name}) => ({name:name}) )
}

const getPokemonsByName = async (name: string): Promise<Pokemon> => {
    try {
        const pokemon: Pokemon = await fetch(`https://pokeapi.co/api/v2/pokemon/${name}`).then(
            res => res.json()
...
export default async function PokemonsName({ params }: PokemonsNameProps) {
